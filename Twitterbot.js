console.log("Gathering Information! Bot startup!");
...
T.get('account/verify_credentials', { skip_statust: true } ,function(err, data, response) {
...
function tweetEvent(eventMsg) { 
  ...
  if (replyto == 'abt_bot') {
    var newtweet = ('@' + screenName + 'like gnuenlyd on facebook!')
    tweetIt(newtweet);
  }
}

tweetIt();
setInterval(tweetIt, 1000*24*3600);
function tweetIt() {
...
var quote = [
  ...
  "Give as few orders as possible, his father had told him once long ago. Once youve given orders on a subject, you must always give orders on that subject. Frank Herbert (from Dune)",
...
var randomquo = quote[Math.floor(Math.random() * (quote.length))];

var tweet = {
  status: '#quoteoftheday ' + randomquo
}
